...
function createInjector(
  ...
  const resolvedProviders = ReflectiveInjector.resolve(
    providers.concat(dynamicProviders)
  );
  if (isPresent(parent)) {
    return parent.createChildFromResolved(
      resolvedProviders
...
describe(`injector`, () => {
  ...
  it("should resolve dependencies based on @Inject annotation", () => {
    ...
    expect(car instanceof CarWithInject).toBeTruthy();
    expect(car.engine instanceof TurboEngine).toBeTruthy();
  });
  ...
  it("should throw when the aliased provider does not exist", () => {
    ...
    const e = `No provider for ${stringify(SportsCar)}! (car -> ${stringify(
      SportsCar
    )})`;
  ...
  it("should show the full path when no provider", () => {
    ...
    expect(() => injector.get(CarWithDashboard)).toThrowError(
      `No provider for DashboardSoftware! (${stringify(
        CarWithDashboard
  ...
  it("should throw when trying to instantiate a cyclic dependency", () => {
    const injector = createInjector([
      Car,
      { provide: Engine, useClass: CyclicEngine },
    ]);
  ...
  it("should instantiate an object after a failed attempt", () => {
    ...
    expect(injector.get(Car) instanceof Car).toBeTruthy();
  });
...
describe("child", () => {
  ...
  it("should not use the child providers when resolving the dependencies of a parent provider", () => {
    ...
    expect(carFromChild.engine instanceof Engine).toBeTruthy();
  });
  ...
  it("should create new instance in a child injector", () => {
    ...
    expect(engineFromChild instanceof TurboEngine).toBeTruthy();
  });
...
describe("depedency resolution", () => {
  ...
  describe("default", () => {
    it("should not skip self", () => {
      ...
      expect(child.get(Car).engine instanceof TurboEngine).toBeTruthy();
    });
  });
});
...
describe("resolve", () => {
  ...
  it("should resolve forward references", () => {
    ...
    expect(
      engineProvider.resolvedFactories[0].factory() instanceof Engine
    ).toBe(true);
    expect(
      brokenEngineProvider.resolvedFactories[0].factory() instanceof Engine
    ).toBe(true);
  ...
  it("should support overriding factory dependencies with dependency annotations", () => {
    ...
    expect(provider.resolvedFactories[0].dependencies[0].key.token).toEqual(
      "dep"
    );
  });
